...
def test_compositional_flow_is_marked_by_a_period():
    ...
    assert unit_token_is_flow(Unit.CURRENCY.PER_MONTH)
    assert not unit_token_is_flow(Unit.CURRENCY)
    assert not unit_token_is_flow(Unit.YEARS)
...
def test_tracer_bullet_mettsim_function_passes():
    ...
    # The engine-level check operates on resolved pint units (internal
    # surface); declarations themselves are `Unit` tokens.
    fail_if_function_unit_is_inconsistent(
...
def test_edge_check_passes_for_consistent_subtree():
    # A per-person monthly rate times a head count is a monthly amount. The
    # head count is dimensionless (GEP 10), so the multiplication is a plain
    # scaling that preserves CURRENCY / month.
    def betrag_m(satz_m: float, anzahl: int) -> float:
        return satz_m * anzahl

    fail_if_unit_edges_are_inconsistent(
        functions={"betrag_m": betrag_m},
        resolved_units={
            "satz_m": parse_unit("CURRENCY / month"),
            "anzahl": UNIT_REGISTRY.dimensionless,
            "betrag_m": parse_unit("CURRENCY / month"),
        },
    )


def test_edge_check_catches_dimensional_clash():
    def bad(rent_m: float, price_per_area: float) -> float:
        return rent_m + price_per_area

    with pytest.raises(UnitConsistencyError, match="bad"):
        fail_if_unit_edges_are_inconsistent(
            functions={"bad": bad},
            resolved_units={
                "rent_m": parse_unit("CURRENCY / month"),
                "price_per_area": parse_unit("CURRENCY / hectare"),
                "bad": parse_unit("CURRENCY / month"),
            },
        )


def test_edge_check_catches_month_year_mismatch():
    def passthrough_m(x_y: float) -> float:
        return x_y

    with pytest.raises(UnitConsistencyError, match="infers"):
        fail_if_unit_edges_are_inconsistent(
            functions={"passthrough_m": passthrough_m},
            resolved_units={
                "x_y": parse_unit("CURRENCY / year"),
                "passthrough_m": parse_unit("CURRENCY / month"),
            },
        )
...
def test_edge_check_on_mettsim_subtree_passes():
    """A partially-annotated mettsim subtree: the acre-cap function checks out."""
    fail_if_unit_edges_are_inconsistent(
        functions={
            "acre_size_after_cap": (acre_size_after_cap.function),
        },
        resolved_units={
            "acre_size": parse_unit("hectare"),
            "cap": parse_unit("hectare"),
            "year_from_which_cap_is_applied": parse_unit("year"),
            "evaluation_year": parse_unit("year"),
            "acre_size_after_cap": parse_unit("hectare"),
        },
    )
...
def test_validate_and_strip_passes_matching_unit():
    tagged = UNIT_REGISTRY.Quantity(np.array([1.0, 2.0]), "CASTAR")
    bare = validate_and_strip_input_quantity(
        tagged, expected_unit=parse_unit("CASTAR"), column_label="wealth"
    )
    assert not isinstance(bare, UNIT_REGISTRY.Quantity)
    assert list(bare) == [1.0, 2.0]


def test_validate_and_strip_passes_bare_array_through():
    arr = np.array([1.0, 2.0])
    assert (
        validate_and_strip_input_quantity(arr, expected_unit=parse_unit("CASTAR"))
        is arr
    )


def test_validate_and_strip_converts_currency():
    # A column tagged in silver_penny is converted to a castar expectation.
    # 4 silver pennies = 1 castar (silver_penny = castar / 4).
    tagged = UNIT_REGISTRY.Quantity(np.array([4.0, 8.0]), "SILVER_PENNY")
    bare = validate_and_strip_input_quantity(
        tagged, expected_unit=parse_unit("CASTAR"), column_label="wealth"
    )
    assert not isinstance(bare, UNIT_REGISTRY.Quantity)
    assert bare == pytest.approx([1.0, 2.0])


def test_validate_and_strip_fails_on_incompatible_dimension():
    # An area tag against a currency expectation cannot be converted.
    tagged = UNIT_REGISTRY.Quantity(np.array([1.0]), "hectare")
    with pytest.raises(UnitConsistencyError, match="not compatible"):
        validate_and_strip_input_quantity(tagged, expected_unit=parse_unit("CASTAR"))
