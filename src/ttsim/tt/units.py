...
def _currency_component_of(units: pint.Unit) -> pint.Unit | None:
    ...
    """
    currency_dimensionality = UNIT_REGISTRY.Quantity(1.0, CURRENCY_TOKEN).dimensionality
...
def _flow_period_of(units: pint.Unit) -> pint.Unit | None:
    ...
    *numerator* time unit (the ``year`` of an age, ``Unit.YEARS``) is not a flow
    ...
    """
    time_dimensionality = UNIT_REGISTRY.Quantity(1.0, "year").dimensionality
...
def validate_and_strip_input_quantity(
    arr: Any,  # noqa: ANN401 (a bare array, pandas Series, or pint Quantity)
    *,
    expected_unit: pint.Unit,
    column_label: str | None = None,
) -> Any:  # noqa: ANN401
    """Dimension-checking boundary conversion (GEP 10): convert a pint tag, then strip.

    This is the *full* Layer-2 boundary check: the tag is converted to the
    column's ``expected_unit`` (resolved in the run currency) and stripped to a
    bare array, and a tag of an incompatible *dimension* fails loudly. It is
    **not** the live boundary path: production canonicalisation
    (:func:`ttsim.interface_dag_elements.processed_data`) calls
    :func:`strip_input_quantity_at_boundary`, which checks only the tag's period
    and converts its currency. Threading each column's declared ``expected_unit``
    to the boundary so this dimension check can cover every input column is
    deferred work (GEP 10 §"Units at the boundary"); this function is the
    building block for it.

    A user *may* attach a pint ``Quantity`` to an input column; a column tagged
    in another currency (e.g. DM against a euro run) is rescaled rather than
    rejected. A bare (untagged) array is returned unchanged.

    Args:
        arr: The user-supplied column — a bare array/Series or a pint Quantity.
        expected_unit: The column's declared unit, resolved in the run currency.
        column_label: Identifier for the column, for error messages.

    Returns:
        A bare array (the converted magnitude if it was tagged, else ``arr``).

    Raises:
        UnitConsistencyError: If a tagged column's unit is dimensionally
            incompatible with ``expected_unit``.
    """
    if not isinstance(arr, pint.Quantity):
        return arr
    where = f" on input column {column_label!r}" if column_label else ""
    try:
        return arr.to(expected_unit).magnitude
    except pint.DimensionalityError as e:
        raise UnitConsistencyError(
            f"pint-tagged input{where} has unit '{arr.units}', which is not "
            f"compatible with the declared unit '{expected_unit}' (in the run "
            f"currency)."
        ) from e
...
def fail_if_function_unit_is_inconsistent(
    *,
    ...
    A low-level primitive (GEP 10): it dry-runs one body in isolation
    via :func:`infer_function_unit` and compares the result to the declaration.
    The whole-environment verification — concolic branch exploration,
    run-time unit-blindness screening, the ``verify_units=False`` opt-out — lives
    in :mod:`ttsim.interface_dag_elements.unit_checks`
    (``fail_if_environment_units_are_inconsistent``) and does not use this
    function. This single-body form is kept for targeted, fixture-level checks.
...
def fail_if_unit_edges_are_inconsistent(
    *,
    functions: Mapping[str, Callable[..., Any]],
    resolved_units: Mapping[str, pint.Unit],
    non_unit_kwargs_by_node: Mapping[str, Mapping[str, Any]] | None = None,
) -> None:
    """Data-independent per-function body + edge-consistency check (GEP 10).

    For each function node, its inputs are wrapped in the *resolved* units of
    their producers (the DAG edges), the scalar body is dry-run in NumPy+pint,
    and the inferred output must match the node's own resolved unit. Feeding the
    producers' declared units into each consumer is the edge-consistency pass:
    a monthly producer reaching a consumer that treats it as yearly is caught by
    the body dry-run.

    No user data or fabricated dataset is needed — only the declared units.

    A low-level primitive: it dry-runs each body once on a single
    representative value. The production check that runs over the
    assembled environment —
    :func:`ttsim.interface_dag_elements.unit_checks.fail_if_environment_units_are_inconsistent`
    — supersedes it with concolic branch exploration and the
    ``verify_units=False`` opt-out, and does not call this function.

    Args:
        functions: Maps each function node's qualified name to its scalar body.
        resolved_units: Maps every node (inputs, params, functions) to its fully
            resolved pint unit (non-time part combined with the flow period).
        non_unit_kwargs_by_node: Per-node extra keyword arguments that are not
            unit-carrying (e.g. ``xnp``), passed through verbatim.

    Raises:
        UnitConsistencyError: If any edge or body is dimensionally unsound or a
            node's body infers a unit other than its declared one. All offending
            nodes are reported together.
    """
    by_node = non_unit_kwargs_by_node or {}
    errors: list[str] = []
    for qname, func in functions.items():
        if qname not in resolved_units:
            continue
        params = set(inspect.signature(func).parameters)
        non_unit = by_node.get(qname, {})
        input_units = {p: resolved_units[p] for p in params if p in resolved_units}
        unresolved = params - set(input_units) - set(non_unit)
        if unresolved:
            errors.append(
                f"{qname}: cannot check — inputs without a resolved unit: "
                f"{', '.join(sorted(unresolved))}."
            )
            continue
        try:
            inferred = infer_function_unit(
                function=func, input_units=input_units, non_unit_kwargs=non_unit
            )
        except UnitInferenceError as e:
            errors.append(f"{qname}: {e}")
            continue
        if not units_are_equivalent(left=inferred, right=resolved_units[qname]):
            errors.append(
                f"{qname}: declares '{resolved_units[qname]}' but its body "
                f"infers '{inferred}'."
            )
    if errors:
        raise UnitConsistencyError(
            "Unit edge-consistency check failed:\n  " + "\n  ".join(errors)
        )
