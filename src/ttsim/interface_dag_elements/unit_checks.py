...
from ttsim.tt.vectorization import recompile_with_logical_ops_as_bitwise
...
#: never carry a unit — the partialled framework arguments (GEP 10), shared with
#: `specialized_environment` so the two cannot drift.
...
def resolved_units(
    ...
    """The resolved pint unit of every annotated node in the environment (GEP 10).
...
def _strip_grouping_levels(unit: pint.Unit, *, denominators_only: bool) -> pint.Unit:
    """Divide grouping-level components out of a unit (GEP 10).
    ...
    *derives* a unit from its source and agg_type; its declared token is checked
    ...
    for qname, obj in env.items():
        ...
        error = _verify_one_body(
            qname=qname,
            function=recompile_with_logical_ops_as_bitwise(obj.function),
            declared=declared,
